...
namespace
{
  ...
  argp_option options[] =
  {
    ...
    {"verbosity", 'v', "verbosity_level", 0,
     "How verbose the program is. 0 - silent, 1 - errors, 2 - warnings, 3 - info, "
     "4 - verbose, 5 - debug, 6 - max. Defaults to 3.", 9},
    {}
  };
  ...
  error_t parse_opt (int key, char *arg, argp_state *state)
  {
    ...
    switch(key)
    {
      case 'm':
      {
        ...
        cfg->img_max_size = std::stoi(match[1]);
        break;
      }
      case 1001:
      {
      ...
      case 'v':
      {
        ...
        std::string tmp(arg);
        if (!std::regex_match(tmp, match, re))
          argp_error(state, "Invalid verbosity level");
        cfg->log_level = std::stoi(match[1]);
        if (cfg->log_level > 6)
          argp_error(state, "Verbosity level out of range");
        break;
      }
      case ARGP_KEY_ARG:
      {
